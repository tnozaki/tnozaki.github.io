...
V1 UNIX では B を置換えるプログラミング言語 C が登場した。
だがカーネルは機種依存コードが多いのでいまだ全てアセンブラである。
...
こいつも実は tools 以下の方が面白いんじゃないか説あると思います。

- apout … PDP-11 上の V7 UNIX/2.11BSD のバイナリエミュレータ
...
6 と 7bit 目が PDP-7 の RTC の I/O レジスタ 6 と 7 番に対応してるのねこれ。
