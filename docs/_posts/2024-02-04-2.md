...
+#define VMT_CLOCK_SYNCTIME	0x1
+	unsigned int		sc_clock_flags;
...
+	if (sc->sc_clock_flags & VMT_CLOCK_SYNCTIME)
+		callout_halt(&sc->sc_clock_sync_tick, NULL);
...
+		if (sc->sc_clock_flags & VMT_CLOCK_SYNCTIME) {
+			callout_halt(&sc->sc_clock_sync_tick, NULL);
+			sc->sc_clock_flags &= ~VMT_CLOCK_SYNCTIME;
+		}
...
+			}
+		} else {
+			if ((sc->sc_clock_flags & VMT_CLOCK_SYNCTIME) == 0) {
+				/* synctime off -> on */
+				callout_schedule(&sc->sc_clock_sync_tick,
+				    mstohz(sc->sc_clock_sync_period_seconds * 1000));
+				sc->sc_clock_flags |= VMT_CLOCK_SYNCTIME;
...
@@ -73,6 +73,7 @@ struct vmt_softc {
 ...
 #define VMT_CLOCK_SYNCTIME	0x1
+#define VMT_CLOCK_TOOLS_ENABLE	0x2
 	unsigned int		sc_clock_flags;
 #define VMT_CLOCK_SYNC_PERIOD_SECONDS 60
 	int			sc_clock_sync_period_seconds;
...
+			if (sc->sc_clock_flags & VMT_CLOCK_TOOLS_ENABLE) {
+				/* time.synchronize.tools.enable on -> off */
+				sc->sc_clock_flags &= ~VMT_CLOCK_TOOLS_ENABLE;
+			}
+		} else {
+			if ((sc->sc_clock_flags & VMT_CLOCK_TOOLS_ENABLE) == 0) {
...
@@ -74,6 +74,7 @@ struct vmt_softc {
 ...
 #define VMT_CLOCK_SYNCTIME	0x1
 #define VMT_CLOCK_TOOLS_ENABLE	0x2
+#define VMT_CLOCK_TOOLS_STARTUP	0x4
 	unsigned int		sc_clock_flags;
 #define VMT_CLOCK_SYNC_PERIOD_SECONDS 60
 	int			sc_clock_sync_period_seconds;
...
+			if ((sc->sc_clock_flags & VMT_CLOCK_TOOLS_STARTUP) == 0) {
+				/* time.synchronize.tools.start off -> on */
+				vmt_sync_guest_clock(sc);
+				sc->sc_clock_flags |= VMT_CLOCK_TOOLS_STARTUP;
+			}
+		}
+		if (vm_rpc_send_str(&sc->sc_tclo_rpc, VM_RPC_REPLY_OK) != 0)
+			sc->sc_rpc_error = 1;
...
@@ -75,6 +75,7 @@ struct vmt_softc {
 ...
 #define VMT_CLOCK_TOOLS_ENABLE	0x2
 #define VMT_CLOCK_TOOLS_STARTUP	0x4
+#define VMT_CLOCK_TOOLS_BACKWARD 0x8
 	unsigned int		sc_clock_flags;
 #define VMT_CLOCK_SYNC_PERIOD_SECONDS 60
 	int			sc_clock_sync_period_seconds;
...
+			sc->sc_clock_flags &= ~VMT_CLOCK_TOOLS_BACKWARD;
+		else
+			sc->sc_clock_flags |= VMT_CLOCK_TOOLS_BACKWARD;
+		if (vm_rpc_send_str(&sc->sc_tclo_rpc, VM_RPC_REPLY_OK) != 0)
+			sc->sc_rpc_error = 1;
...
ちゅーかんじで、ドキュメントすらない tc_setclock() というAPIで時刻が逆行しようがお構いなしに時刻同期してるので、常時 time.synchronize.tools.startup.backward は true であるといえる。
