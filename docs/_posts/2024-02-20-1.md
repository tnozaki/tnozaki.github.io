...
- a7out … PDP-7 ユーザーモードシミュレータ 直接 *.s を実行可能
- as7 … PDP-7 アセンブラ
- ccov7 … PDP-7 カバレッジ
...
ただ上記のコードにおいて MQ レジスタはまったく無関係なので、単に戻り値を返すためだけにも使われてるって感じ。


## 再び pibreak ラベルを読む

さて `s.tim` が現在時刻情報とということが time(2) から判明した。
改めて `pibreak` を追っていくと
...
で、現代的な UNIX と同様に秒単位でカウントしてると勘違いした記述があった事に謝罪したい(結論自体は変わらないのだが)。
えーと切腹でいいですか、痛いのヤダからスゥと意識が飛ぶ首吊りで勘弁願いたいところである、というか究極の鎮痛剤であるショットガンいいっすか。
